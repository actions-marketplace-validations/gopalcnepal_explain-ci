...
def _parse_log_lines(raw: str, fallback: int = 1000) -> int:
    """Parse the log_lines input into a positive line count.

    Args:
        raw: Raw input value from the environment.
        fallback: Value used when raw is missing, invalid, or non-positive.

    Returns:
        Positive number of log lines to keep per section.
    """
    try:
        value = int(raw)
    except ValueError:
